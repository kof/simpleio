...
function Connection(params, server) {
    ...
    this._subscription = this.adapter.subscribe(query, function(message) {
        self.multiplexer.add(message);
    });

    // Load messages user has probably missed during reconnect.
    this.adapter.get(query, function(err, messages) {
...
Connection.prototype.respond = function(reason) {
    ...
    this.adapter.delivered(ids, true, false, function(err) {
...
Connection.prototype.close = function() {
    ...
    this.server.disconnect(this._clientId);
