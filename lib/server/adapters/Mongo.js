...
function Mongo(uri, opts) {
    ...
    this.client = mubsub(uri || opts.uri, opts.db)
...
Mongo.prototype.open = function(sender, callback) {
    this.channel.publish('open', {sender: sender, date: new Date}, callback);

    return this;
};

/**
 * Get users who opened a connection in last x ms.
 *
 * @param {Date} since in ms
 * @param {Function} callback
 * @return {Mongo} this
 */
...
Mongo.prototype._ensureIndex = function() {
    this.client.db.ensureIndex(this.options.name, {
        'message.recipient': 1,
        'message.deliverable': 1
    }, this._error.bind(this));

    this.client.db.ensureIndex(
        this.options.name,
        {'message.date': 1},
        this._error.bind(this)
    );
